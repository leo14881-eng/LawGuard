...
1. 阅读 `LAWGUARD_SOT.md`（长期产品事实）与 `docs/project/AUTODEV_PROGRESS.md`（真实
   开发状态，见其中的功能状态表）；
2. 用 Glob/Grep 等工具搜索现有路由、页面、组件、组合式函数、工具类、数据模型和测试，
   确认需求是否已有实现；
3. 判断需求属于：新功能 / 已有功能扩展 / 已有功能修复 / 已完成无需修改；
4. 如已有相同或高度相似实现，必须复用或修改现有实现，禁止新建重复页面、重复组件、
   重复工具类或重复状态模型；
5. 不得仅凭对话 Prompt 就认定某功能不存在，也不得仅凭 `LAWGUARD_SOT.md` 记录了产品
   定位就认定对应代码已经实现——是否完成以 `docs/project/AUTODEV_PROGRESS.md`、实际
   代码、测试结果和 `npm run build` 为准；
6. 开发完成后：检查是否形成新的长期产品事实（有则更新 `LAWGUARD_SOT.md`）、更新
   `docs/project/AUTODEV_PROGRESS.md` 的真实状态与相关文件/测试/Build 结果、确认未
   引入重复实现、确认未破坏原有功能、执行测试和 Build；
7. 不得删除或覆盖已有长期功能，除非当前任务明确要求取消或替换，并记录迁移说明。
8. 任何新页面开发前，必须先确认所属页面类型，并复用现有页面骨架、Design Tokens 和
   统一组件（见 `LAWGUARD_SOT.md` 第 12.1～12.3 节）。禁止页面自行创造新的颜色、
   间距、按钮、卡片和提示样式。

状态定义统一使用：`PLANNED`（已规划）/ `IN_PROGRESS`（开发中）/ `IMPLEMENTED`（已实现
待验证）/ `VERIFIED`（已完成并验证）/ `BLOCKED`（已阻塞）/ `REJECTED`（已取消），禁止
使用"差不多完成""基本完成""应该完成"等模糊表述；仅当代码存在、关键测试通过、Build
通过、必要页面已人工检查、无已知阻塞问题时，才可标记为 `VERIFIED`。
